...
#include "twi_lcd.h"
#include "DS1302.h"
unsigned char Read_E2PROM(unsigned int uiAddr)
{ while(EECR & (1<<EEWE)); /*等待上次写结束*/
	EEAR = uiAddr; /*设置读取EEPROM的地址*/
	EECR |= (1<<EERE); /*通过EERE位读取EEPROM,*/
	return EEDR; /*返回读取的数据*/
}
void Write_E2PROM(unsigned int uiAddr,unsigned char ucData)
{ while(EECR & (1<<EEWE)); /*等待上次写结束*/
	EEAR = uiAddr; /*设置要写EEPROM的地址*/
	EEDR = ucData; /*要写的数据存入寄存器*/
	EECR |= (1<<EEMWE); /*EEMWE置为‘1’，*/
	EECR |= (1<<EEWE); /*通过EEWE位写EEPROM，*/
}
#include <reg52.h>
#include <intrins.h>
